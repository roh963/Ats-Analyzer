...
from typing import List, Dict, Any, Optional           # type hints for function signatures
from backend.models.schemas import IssueDetail         # custom class that defines one issue's structure


"""
This is the main function that checks a resume for 10 common problems (like missing sections, weak action verbs, no numbers in achievements, etc.). For each problem found, it creates a detailed IssueDetail explaining what's wrong and how to fix it, then returns the full list of problems.
"""
def analyze_issues(
        resume_text: str, 
        parsed_resume: Dict, 
    ...
    detected: List[IssueDetail] = []  # empty list to store all found issues

    # get only valid dictionary entries from each section
    ...
    # get summary text, "or ''" avoids error if it's None
    summary = (parsed_resume.get('professional_summary') or '').strip()

    # join all experience descriptions into one text block for regex checks
    experience_text = '\n'.join(e.get('description', '') for e in exp_entries).strip()

    # ---------- CHECK 1: Missing Projects Section ----------
    resume_lower = resume_text.lower()
    # look for words that suggest projects exist, even outside a formal section
    ...
    # ---------- CHECK 2: Missing Experience Section ----------
    ...
    # ---------- CHECK 3: Missing Education Section ----------
    ...
    # ---------- CHECK 4: Missing or Weak Skills Section ----------
    # only flag if BOTH parsed skills field is empty AND extracted skill count is low
    ...
    # ---------- CHECK 5: Skills Lack Supporting Evidence ----------
    ...
    # flag if more skills are unproven than proven
    if total_skills > 0 and len(unvalidated) > len(validated):
        unsupported_list = ', '.join(unvalidated[:8])   # show first 8 only
    ...
    # ---------- CHECK 6: Weak Action Verbs ----------
    # collect every non-empty bullet line from experience descriptions
    description_lines = [
        line.strip()
    ...
    # flag if there are enough bullets but very few use strong action verbs
    ...
    # ---------- CHECK 7: No Quantifiable Achievements ----------
    number_pattern = r'\d+[%+]?|\$\d+'   # matches numbers, %, + signs, or dollar amounts
    ...
    # ---------- CHECK 8: Missing Contact Information ----------
    if contact_info:
        missing_contacts = []
        ...
        # only flag if 2 or more contact details are missing
    ...
    # ---------- CHECK 9: Low Formatting Score ----------
    formatting_score = scores.get('formatting_score', 20)   # default to 20 (max) if missing
    ...
    # ---------- CHECK 10: Missing Summary/Objective ----------
    if not summary:
        detected.append(IssueDetail(
    ...
    return detected   # send back the full list of found issues



"""
This function takes the full list of detected resume issues and creates a shorter list containing just the titles of each issue. It's used to quickly show what problems were found, without all the detailed explanations.
"""
def generate_issues_summary(detected_issues: List[IssueDetail]) -> List[str]:
    ...
    # go through each issue and pull out only its title field
