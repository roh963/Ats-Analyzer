...
from sentence_transformers import SentenceTransformer   # generates embeddings for similarity checks
from typing import Dict, List, Optional          # type hints for readability

from backend.models.schemas import IssueDetail                              # data structure for one detected issue
from backend.services.groq_parser import parse_resume, parse_job_description  # extract structured data from raw text
from backend.services.jd_matcher import compare_resume_with_jd               # compares resume against a job description
from backend.services.feedback_engine import analyze_issues, generate_issues_summary  # detects and summarizes issues
from backend.services.ats_scorer import calculate_overall_score, validate_skills_with_projects  # scoring logic



"""
This is the top-level function that runs the entire resume analysis process from start to finish — parsing the resume, validating skills, comparing against a job description (if given), calculating scores, detecting issues, and packaging everything into one final result.
"""
...
def analyze_full_resume(
    ...
    # Step 1: use AI (Groq) to extract structured data from raw resume text
    ...
    # add up total months of experience across all jobs
    ...
    # Step 2: check which skills are backed by real projects/experience
    ...
    # Step 3: if a job description was given, compare resume against it
    jd_comparison_result = None
    jd_keywords = None
    if job_description and job_description.strip():
        parsed_jd = parse_job_description(job_description.strip())
        # combine all JD keyword sources into one list, remove duplicates
    ...
    # Step 4: get default grammar/location results (likely filled in elsewhere in pipeline)
    ...
    # Step 5: calculate the overall ATS score using everything gathered so far
    ...
    # Step 6: detect specific issues (missing sections, weak verbs, etc.)
    ...
    # Step 7: turn detailed issues into a short list of titles
    issues_summary = generate_issues_summary(detailed_feedback)

    # Step 8: calculate skill validation percentage for the final report
    ...
    # Step 9: package everything into one final result dictionary
    return {
        "ATS_score":          scores['overall_score'],
        "ats_score":          scores['overall_score'],   # duplicate key for naming compatibility
        "component_scores": {
        ...
        "jd_comparison":     jd_comparison_result,   # duplicate key for naming compatibility
        "skills":            skills,
        "matched_keywords":  (
            jd_comparison_result['matched_keywords']
            if jd_comparison_result else list(keywords[:20])   # fallback if no JD given
        ),
        "missing_keywords":  (
...
"""
This function scans the parsed resume data and scores to build a list of positive points about the resume — what it's already doing well. Used to give encouraging feedback alongside the issues found.   
"""
def _generate_strengths(
    ...
    strengths = []   # empty list to collect positive points

    # check presence of each major resume section
    ...
    # praise if resume has a strong number of skills
    if len(skills) >= 8:
        strengths.append(f"Strong skill set — {len(skills)} skills detected")

    # praise if resume uses enough action verbs
    if len(action_verbs) >= 5:
        strengths.append(f"Uses {len(action_verbs)} strong action verbs in bullet points")

    # praise if enough skills are proven by real projects/experience
    ...
    # praise high formatting and content scores
